...
[
    {
        "name": "uwsgi",
        "image": "gerey/pixelgroup-home-uwsgi:$tag",
        "essential": true,
        "memory": 200,
        "cpu": 10
    },
    {
        ...
        "image": "gerey/pixelgroup-home-nginx:$tag",
        "portMappings": [
            {
                "containerPort": 8000,
           	"hostPort": 80
...
deploy() {
    service="$1"
    family="pixelgroup-home"
    cluster="pixelgroup-home"
    tag="${CIRCLE_SHA1}"
...
case "$1" in
    staging) deploy "pixelgroup-home-staging" ;;
    prod) deploy "pixelgroup-home" ;;
esac
